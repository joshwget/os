...
full_cloud_config_path = './tests/integration/assets/test_01/cloud-config.yml'
...
def qemu(request):
    q = u.run_qemu(request, ['--append', 'rancher.log', '--cloud-config', cloud_config_path] + net_args[u.arch])
...
@pytest.fixture(scope="module")
def cloud_config():
    return yaml.load(open(full_cloud_config_path))
